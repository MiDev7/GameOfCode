...
try {
  client.connect().then(() => {
    ...
    router.get("/GetScenarioByLanguage", async (req, res) => {
      ...
      const response = await getDailyScenario(language);
      res.json(response);
    });
  });
} catch (error) {
